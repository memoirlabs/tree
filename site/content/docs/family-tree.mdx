...
The layout prevents same-row card overlap and keeps child groups near their visible parent anchors. Descendant rows omit a descendant's own co-parent by default; set `layoutPolicy.descendantCoparents` to `"include"` when that co-parent should be a visible row participant. Large blended families can become wide; use `limits` to control the visible neighborhood.
...
Use `layoutPolicy` for app-specific genealogy choices. By default, multiple subject partners are balanced around the subject and a descendant's co-parent is not promoted into the visible descendant row.
