...
A family tree graph has six concepts:
...
Unknown parent slots are explicit graph facts, not automatic spouse bars. Add
the placeholder person, its unknown group and parent-child link, and structural
metadata describing the slot:
...
The unknown group keeps the placeholder in layout without drawing a horizontal
partnership edge. The metadata produces stable structural data attributes and
the default accessible relationship wording `"parent"`. There is no combined
relationship token for this case.

Tree does not infer placeholder semantics from IDs, names, labels, or app-owned
flags. Pass `shouldRenderPersonCard` only when a person should act as a
connector point. The layout still uses that person for parentage and edge
routing, but the card layer does not render a full card:
...
If the unknown placeholder is a parent of a child, include its own
`parentChildLink`, as in the complete example above.
...
Graph-mode cards also receive placement and person metadata:
...
Use `slotRole: "parent"` for a dotted missing-parent card in a
child-bearing group. Tree uses that explicit metadata in its default card and
accessible relationship wording; it does not guess the role from an ID or
display name.

Use `placement` and `metadata` when your card needs to open the exact relationship editor or add/link flow that placed the person. Labels are for display; mutation intent should come from structural metadata.

Use `getRelationLabel` when app wording differs from Tree's defaults. Check
explicit slot metadata before applying a broader label override:
...
Returning `undefined` delegates to Tree's metadata-aware default wording.
...
The layout prevents same-row card overlap and keeps child groups near their visible parent anchors. Descendant rows include a descendant's own co-parent by default so child groups stay attached to the child-bearing union. Set `layoutPolicy.descendantCoparents` to `"omit"` when that co-parent should stay outside the rendered neighborhood. Large blended families can become wide; use `limits` to control the visible neighborhood.
...
Use `layoutMode="compact-family"` for compact subject-centered trees. It keeps the subject centered and places spouse/partner cards adjacent to the subject. Layout mode changes placement only; it does not hide cards.

Use `layoutPolicy` for app-specific genealogy choices. By default, multiple subject partners are balanced around the subject and a descendant's co-parent is promoted into the visible descendant row.
