...
Current npm release: [`@memoir/tree@0.8.0`](https://www.npmjs.com/package/@memoir/tree/v/0.8.0).
...
Graph mode is the best starting point for app code. It is just JSON-shaped state: people, a subject, partnership groups, parent-child links, optional guardianship links, and optional structural person metadata.
...
Pass `estimatedCardSize` when your custom family cards differ from the default first-pass estimate of `220x80`. Use `shouldRenderPersonCard` when a person should remain as a connector-only layout point instead of a visible card; `layoutMode="compact-family"` changes placement only. Use `layoutPolicy` to omit descendant co-parent cards or to place all subject partners after the subject.
