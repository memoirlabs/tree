...
Pass `spacing` when your custom cards need a different density. The default `interactionMode` is `"pan"`; users can drag the canvas or non-interactive parts of cards. Buttons, links, inputs, selects, textareas, contenteditable elements, and elements marked with `data-tree-drag-ignore` keep their own pointer behavior.
