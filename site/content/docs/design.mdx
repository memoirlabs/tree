...
The default `interactionMode` is `"pan"`, so users can drag the canvas and non-interactive card areas. Native controls inside cards keep their own pointer behavior. Add `data-tree-drag-ignore` to any custom card element that should not start panning.
