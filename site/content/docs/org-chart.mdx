...
function LinkedTeamCard({ canEdit, href, person, ...rootProps }: OrgCardProps<Person> & TeamCardExtras) {
  return (
    <article {...rootProps}>
      <a href={href}>{person.name}</a>
      {canEdit ? <small>Editable</small> : null}
    </article>
  );
}

<OrgChart<Person, TeamCardExtras>
  people={people}
  root="ceo"
  relationships={relationships}
  card={LinkedTeamCard}
