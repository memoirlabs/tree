...
It uses the same tree core as `FamilyTree`: measured cards, SVG edges, pan, zoom, accessible card props, custom React cards, and the tiny stylesheet.
...
`rootProps` includes the accessibility props, keyboard handlers, stable `data-*` attributes, and click handlers that belong on the card root.
