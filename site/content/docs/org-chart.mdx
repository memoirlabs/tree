...
Override `spacing` when your custom org cards need more or less room. The default `interactionMode` is `"pan"`; users can drag the canvas and non-interactive card areas. Use `"pan-page-scroll"` when vertical touch gestures should scroll the page. Native controls inside custom cards keep their own pointer behavior.
