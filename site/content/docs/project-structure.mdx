...
Keep generated family slots in the same graph data module. Represent a missing
parent with `personMetadata` using `kind: "unknown-slot"` and
`slotRole: "parent"`; do not parse IDs, names, or rendered relationship labels
to recover structural intent in actions.
