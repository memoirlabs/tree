...
By default, a visible child's own child-bearing partner or co-parent is promoted into the descendant row so the child remains attached to the child-bearing union. Set `layoutPolicy.descendantCoparents` to `"omit"` when that co-parent should stay outside the rendered neighborhood; siblings who are not part of that next-generation group remain outside the couple/co-parent cluster.
...
Use `layoutPolicy` when your app needs more specific genealogy placement. By default, multiple subject partners are balanced around the subject and a descendant's co-parent is promoted into the visible descendant row. Opt into the older compact behavior when useful:
